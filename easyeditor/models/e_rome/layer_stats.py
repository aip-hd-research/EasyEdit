...
def layer_stats(
    ...
    """
    logger = logging.getLogger("rome.layer_stats.layer_stats")

    def get_ds():
        SUBSETS = dict(wikitext="wikitext-103-raw-v1", wikipedia="20220301.en")
        FIELDS = dict(
            wikitext="text", wikipedia="text", **{"codeparrot/github-code": "code"}
        )
        subset = SUBSETS[ds_name] if ds_name in SUBSETS else None
        logger.info(f"ds_name: {ds_name} | subset: {subset}")
        ...
        raw_ds = Dataset.from_list(
            [row for row in raw_ds.shuffle().take(sample_size)],
    ...
    if cache_key is None:
        cache_key = f"{model_name}/{ds_name}_stats/{layer_name}_{precision}_{'-'.join(sorted(to_collect))}{size_suffix}"
    ...
    loader = tally(
        stat,
        ds,
        cache=(filename if do_cache else None),
        ...
        for batch_group in progress(loader, total=batch_count):
            for batch in batch_group:
                ...
                if torch.isinf(feats).any():
                    logger.info("Skipping features containing infinity")
                    continue
